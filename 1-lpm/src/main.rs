...
mod pressure_modulator;

use core::arch::asm;
use core::panic::PanicInfo;
use crate::fractal_feedback::FractalManifold;
use crate::pressure_modulator::PressureModulator;

static mut MANIFOLD: FractalManifold = FractalManifold::new();
static mut MODULATOR: PressureModulator = PressureModulator::new();
...
_st: *const core::ffi::c_void) -> usize {
    ...
    loop {
        let jitter = read_jitter();
        let resonance = MANIFOLD.calculate_fractal_step(jitter);
        let _repel = MODULATOR.modulate_lattice(jitter, &mut 
MANIFOLD.drift);
